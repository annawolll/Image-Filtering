...
void podzial_na_skladowe(int wier, int kol, unsigned char* obraz, unsigned char** b, unsigned char** g, unsigned char** r) {
	int licznik = 0;
	//Rozdzielenie obrazu wejœciowego na 3 tablice sk³adowych jego pikseli w kolejnoœci B,G,R z uwzglêdnieniem wymiarów obrazu
...
bool zapis_obrazu(unsigned char* obraz, char* nazwa_pliku_wy, char* nazwa_pliku_we) {
	...
	byte null = 0;
	for (int i = 0; i < roznica; i++)//Nadpisanie pustych bajtów
